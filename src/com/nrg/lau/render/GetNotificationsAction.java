...
public class GetNotificationsAction implements View {

	private static Logger log = Logger.getLogger(GetNotificationsAction.class);
	...
	try{
		log.info("ENTER - GetNotificationsAction render()");
...
//		pw.write(readData("/getFieldHistory.do").toString());
		...
		pw.flush();
		pw.close();
		log.info("EXIT - GetNotificationsAction render()");
	}catch(Exception e){
		log.error( e, e);
	}
}
	...
	private StringBuffer readData(String fileName) throws IOException {
		...
		URL fileURL = GetNotificationsAction.class.getResource(fileName);
...
private StringBuffer getData() {
		...
		try {
			...
			String sql 	= "{call ? := LAU_NOTIFICATION.GetUserNotifications(?)}";
		...
		log.info("GetNotificationsAction" + tmp);
		return tmp;
	}
}
