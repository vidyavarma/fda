...
public class DeleteE2BNodeAction implements View {

	private static Logger log = Logger.getLogger(DeleteE2BNodeAction.class);
	...
	public void setDataSource(DataSource dataSource) {
		...
		this.ds = dataSource;
		log.info("get DeleteE2BNode -Initialize db template()");
	}

	public void render(Map model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		log.info("ENTER - DeleteE2BNode render()");
		...
		if (Constants.DEBUG) {
			...
			pw.write(readData("/deleteE2BNode.do").toString());
		} else {
		...
		log.info("EXIT - DeleteE2BNode render()");
	}

	private StringBuffer readData(String fileName) throws IOException {

		StringBuffer sb = new StringBuffer();
		URL fileURL = DeleteE2BNodeAction.class.getResource(fileName);
		...
		reader.close();
		log.info("DeleteE2BNodeAction: " + sb.toString());
		return sb;
		// return sb.toString();
	}
	...
	private StringBuffer getData() throws Exception {
		...
		try {
			...
			String sql = "{call ? := LAU_UTL.deleteE2bNodes(?,?,?)}";
