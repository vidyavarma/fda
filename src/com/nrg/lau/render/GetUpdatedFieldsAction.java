...
public class GetUpdatedFieldsAction implements View {

	private static Logger log = Logger.getLogger(GetUpdatedFieldsAction.class);
...
//	private String userId	= "";
	...
	try{
		log.info("ENTER - GetUpdatedFieldsAction render()");
//		userId = CommonDAO.getUSERID(request);
		reportId = (String) request.getParameter("reportId");
		selectedTime = (String) request.getParameter("selectedTime");
		response.setContentType(Constants.CONTENT_TYPE);
		response.setHeader("Cache-Control", Constants.CACHE_CONTROL);
		PrintWriter pw = response.getWriter();
//		pw.write(readData("/getUpdatedFields.do").toString());
		pw.write(getData().toString());	
		pw.flush();
		pw.close();
		log.info("EXIT - GetUpdatedFieldsAction render()");
	}catch(Exception e){
		log.error( e, e);
	}
}
	...
	private StringBuffer readData(String fileName) throws IOException {
		...
		URL fileURL = GetUpdatedFieldsAction.class.getResource(fileName);
...
private StringBuffer getData() {
		...
		try {
			...
			String sql 	= "{call ? := LAU_UTL.getUpdatedReportFields(?,?)}";
...
//			stmt.setString(2,userId);
		...
		log.info("GetUpdatedFieldsAction" + tmp);
		return tmp;
	}
}
/*
