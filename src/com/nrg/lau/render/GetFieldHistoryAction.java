...
public class GetFieldHistoryAction implements View {

	private static Logger log = Logger.getLogger(GetFieldHistoryAction.class);
	...
	try{
		log.info("ENTER - GetFieldHistoryAction render()");
...
//		pw.write(readData("/getFieldHistory.do").toString());
		...
		pw.flush();
		pw.close();
		log.info("EXIT - GetFieldHistoryAction render()");
	}catch(Exception e){
		log.error( e, e);
	}
}
	...
	private StringBuffer readData(String fileName) throws IOException {
		...
		URL fileURL = GetFieldHistoryAction.class.getResource(fileName);
...
private StringBuffer getData() {
		...
		try {
			...
			String sql 	= "{call ? := LAU_UTL.getFieldHistory(?,?)}";
...
//			stmt.setString(2,userId);
		...
		log.info("GetFieldHistoryAction" + tmp);
		return tmp;
	}
}
/*
