...
public class GetHistoryStatusAction implements View {

	private static Logger log = Logger.getLogger(GetHistoryStatusAction.class);
...
//	private String fieldId	= "";
	...
	try{
		log.info("ENTER - GetHistoryStatusAction render()");
...
//		pw.write(readData("/getHistoryStatus.do").toString());
		pw.write(getData().toString());	
		pw.flush();
		pw.close();
		log.info("EXIT - GetHistoryStatusAction render()");
	}catch(Exception e){
		log.error( e, e);
	}
}
	...
	private StringBuffer readData(String fileName) throws IOException {
		...
		URL fileURL = GetHistoryStatusAction.class.getResource(fileName);
...
private StringBuffer getData() {
		...
		try {
			...
			String sql 	= "{call ? := LAU_UTL.getReportUpdateHistory(?)}";
...
//			stmt.setString(3,recordId);
		...
		log.info("GetHistoryStatusAction" + tmp);
		return tmp;
	}
}
/*
