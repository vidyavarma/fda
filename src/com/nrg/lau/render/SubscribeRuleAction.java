...
public class SubscribeRuleAction implements View {

	private static Logger log = Logger.getLogger(SubscribeRuleAction.class);
	...
	try{
		log.info("ENTER - SubscribeRuleAction render()");
		...
		pw.flush();
		pw.close();
		log.info("EXIT - SubscribeRuleAction render()");
	}catch(Exception e){
		log.error( e, e);
	}
}
...
private StringBuffer getData() {
		...
		try {
			...
			String sql 	= "{call ? := LAU_NOTIFICATION.subscriberule(?,?)}";
		...
		log.info("SubscribeRuleAction" + tmp);
		return tmp;
	}
}
