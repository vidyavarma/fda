...
public class GetTextOperator implements View {

	private static Logger log = Logger.getLogger(GetTextOperator.class);
	...
	//	if (Constants.DEBUG)
			pw.write(readData("/getTextOperator.do").toString());
	//	else
	//		pw.write(getData().toString());	
		pw.flush();
		pw.close();

		log.info("EXIT - GetTextOperator render()");
	}
	public void setDataSource(DataSource dataSource) {
		new SimpleJdbcTemplate(dataSource);
		this.ds = dataSource;
		log.info("GetTextOperator -Initialize db template()");
	}
	private StringBuffer readData(String fileName) throws IOException {

		StringBuffer sb = new StringBuffer();
		URL fileURL = GetTextOperator.class.getResource(fileName);
