...
public class GetCheckBoxOperator implements View {

	private static Logger log = Logger.getLogger(GetCheckBoxOperator.class);
	...
	public void render(Map model, HttpServletRequest request,
		...
		log.info("ENTER - GetCheckBoxOperator render()");
	...
	//	if (Constants.DEBUG)
			pw.write(readData("/getCheckBoxOperator.do").toString());
	//	else
	//		pw.write(getData().toString());	
		pw.flush();
		pw.close();

		log.info("EXIT - GetCheckBoxOperator render()");
	}
	public void setDataSource(DataSource dataSource) {
		new SimpleJdbcTemplate(dataSource);
		this.ds = dataSource;
		log.info("GetGetCheckBoxOperator -Initialize db template()");
	}
	private StringBuffer readData(String fileName) throws IOException {

		StringBuffer sb = new StringBuffer();
		URL fileURL = GetCheckBoxOperator.class.getResource(fileName);
