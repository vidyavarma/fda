...
public class GetUserListAction implements View {

	private static Logger log = Logger.getLogger(GetUserListAction.class);
	...
	public void render(Map model, HttpServletRequest request,
		...
		log.info("ENTER - GetUserListAction render()");
	...
	//	groupId = CommonDAO.getUSERGROUPID(request);
		...
		log.info("EXIT - GetUserListAction render()");
	}

	public void setDataSource(DataSource dataSource) {
		new SimpleJdbcTemplate(dataSource);
		this.ds = dataSource;
		log.info("UserList -Initialize db template()");
	}
	...
	private StringBuffer getData() {
		...
		try {
			...
			String sql = "{call ? := LAU_UTL.getuserlist()}";
